...
<th class:sortable={field} class:active={sort.isActive}>
	<Dropdown position="bottom-start">
		...
		{#snippet content()}
			<aside class="z-depth-1">
				<button class="btn" onclick={() => sort.asc()}
					><span>{@html glyph.asc}</span> Asc</button
				>
				<button class="btn" onclick={() => sort.desc()}
					><span>{@html glyph.desc}</span> Desc</button
				>
				<div class="divider"></div>
				<button class="btn" onclick={() => view.toggle(field)}
					><span>{@html glyph.hide}</span> Hide</button
				>
			</aside>
		{/snippet}
	</Dropdown>
