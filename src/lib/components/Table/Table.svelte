<script>
	...
	import {
		...
		getFilteredSpecies,
		loadSpecies,
	} from '$lib/stores/datatables/species.svelte.js';
	...
	import {
		...
		selGrid,
		paramsFilter,
	} from '../../stores/new_sidebar_stores.js';
    ...
    search = table.createSearch(['year','month','species_n','name_it','valley']) */
		if (allids.length > 0) console.log('allids.length', allids.length);
  ...
  return hasChanged
	...
	$effect(() => {
		...
		if (allids.length > 0) {
			...
			hasChanged =
				currentIds.length !== previousAllIds.length ||
			...
			if (hasChanged && currentIds.length > 0) {
				...
				phpFetchComplete = false; // Reset before starting new fetch
				...
				fetchDataFromPHP(currentIds).then(() => {
					...
					if (info && info.length > 0) {
						...
						filteredTaxDataObjs = taxDataObjs.filter((obj) =>
							allids.includes(obj.tax_id)
						);
						console.log(
							'Table: filteredTaxDataObjs, containing pngp_data',
							filteredTaxDataObjs
						);
					}
				});
	...
	let duplicateTaxDataObjs = $derived.by(() => {
		...
		if (!duplIds || duplIds.length == 0) return;
		...
		if (taxDataObjs && taxDataObjs.length > 0) {
			...
			return taxDataObjs.filter((obj) => allids.includes(obj.tax_id));
		} else {
			return [];
		}
	});
	// Track when PHP fetch is complete
	let phpFetchComplete = $state(false);
	let months = [
		'January',
		'February',
		'March',
	...
	// Create merged data for the table
	// Move infoMerged outside the effect to proper scope
	let infoMerged = $derived.by(() => {
		console.log('infoMerged: checking conditions', {
			phpFetchComplete,
			tableDataLength: tableData?.length,
			allidsLength: allids?.length,
			taxDataObjsLength: taxDataObjs?.length,
		});

		// Only process after PHP fetch is complete
		if (!phpFetchComplete) {
			console.log('infoMerged: waiting for PHP fetch to complete');
			return [];
		}

		// Early return if essential data is missing
		if (!tableData || tableData.length === 0) {
			console.log('infoMerged: tableData not ready');
			return [];
		}

		if (!taxDataObjs || taxDataObjs.length === 0) {
			console.log('infoMerged: taxDataObjs not ready');
			return [];
		}

		if (!allids || allids.length === 0) {
			console.log('infoMerged: allids not ready');
			return [];
		}

		console.log('infoMerged: Processing data AFTER PHP fetch', {
			tableDataLength: tableData.length,
			allidsLength: allids.length,
		});

		const filteredTaxDataObjs = taxDataObjs.filter((obj) =>
			allids.includes(obj.tax_id)
		);

		return tableData.map((item, i) => {
			let tax = sanitizeData(filteredTaxDataObjs).find(
				(obj) => obj.tax_id === item.tax_id
			);
			if (i < 2) console.log('Merging item', item, 'with tax', tax);
			let pngp_data = tax?.pngp_data;
			let s_data = sanitizeData([pngp_data]);

			// Create a new object with transformed month instead of mutating
			const transformedItem = {
				...item,
				month:
					item.month == -1 || item.month == 'No data'
						? item.month
						: months[item.month - 1],
			};
	...
	let table = $derived.by(() => {
		if (!infoMerged || infoMerged.length === 0) {
			console.log('table: infoMerged not ready, returning null');
			return null;
		}

		console.log('table: Creating TableHandler with', infoMerged.length, 'rows');
		return new TableHandler(infoMerged, {
			rowsPerPage: 20,

			highlight: true,
		});
	});
	const total = $derived(table?.rowCount);

	// React to remoteDataStore changes
	$effect(() => {
		const remoteData = $remoteDataStore;
		console.log(
			'Table: remoteDataStore changed in TAble, length:',
			remoteData.length
		);
	...
	function sanitizeData(rows) {
		...
		for (let i = 0; i < rows.length; i++) {
			...
			for (const key in original) {
				row[key] =
					original[key] === null || original[key] === -1
						? replacement
						: original[key];
			}
	...
	async function fetchDataFromPHP(ids) {
		try {
			...
			const response = await fetch(
				`https://pere.gis-ninja.eu/pngp_phps_new/getPointsById.php?${queryParams}`,
				{
					method: 'GET',
				}
			);
			...
			if (response.ok) {
				...
				phpFetchComplete = true; // Signal that PHP fetch is complete
			} else {
	...
	{#if table}
		<aside class="flex">
			<div>
				...
				<div><span class="text-lg">{total.total}</span> records</div>

				{#if duplicateTaxDataObjs && duplicateTaxDataObjs.length > 0}
					<div class="flex text-sm text-center flex-row gap-2">
			...
			<table>
				<thead>
                    ...
                    <div class="flex">
						...
						<MyThFilter {table} field="species_n" name="Scientific Name"
						></MyThFilter>

						<MyThFilter {table} field="name_it" name="Common Name"></MyThFilter>

						<MyThFilter {table} field="total_ind" name="Total Individuals"
						></MyThFilter>

						<MyThFilter {table} field="valley" name="Valley"></MyThFilter>

						<MyThFilter {table} field="year" name="Year"></MyThFilter>

						<MyThFilter {table} field="month" name="Month"></MyThFilter>

						<MyThFilter {table} field="geo" name="Georeferenced"></MyThFilter>
            ...
            {@const displayName = (() => {
                ...
                return nameIt || nameEng || 'No data';
                        ...
                        <div class="flex">
							...
							<td>
								<div class="flex">{@html row.year}</div>
							</td>
							<!--  <td>
                        <div class="flex">{capitalize(row.tax_level)}</div>
                    </td> -->
							<td>
								<div class="flex">{@html row.month}</div>
							</td>
							<td>
								<div class="flex">{@html row.georef}</div>
							</td>
							<!--  <td>
	...
	{:else}
		<aside class="flex">
			<section>
				...
				<div class=" text-xl">
					Apply filters on the map to see data in form of a table
				</div>
			</section>
		</aside>
	{/if}
