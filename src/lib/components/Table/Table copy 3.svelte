...
let previousAllIds = $state([]);
  ...
  $effect(() => {
    ...
    const hasChanged = currentIds.length !== previousAllIds.length ||
   ...
   let table = $derived.by(()=>{
    ...
    if (info && info.length > 0) {
      ...
      let infoMerged=info.map(item=>{
        let tax=filteredTaxDataObjs.find(obj=>obj.id===item.tax_id);
...
return merged;
    ...
    let table = new TableHandler(infoMerged, { rowsPerPage: 10 })
    return table;
    }
...
{#if table}
		...
		<div>
			...
			<p>Here's the species observatsion data for analysis!</p>
...
<div class="h-full flex flex-col items-center justify-center">
    <div class=" text-3xl outline p-2 bg-amber-500 text-white mt-4">No data</div>
    <div class=" text-2xl">Apply filters on the map to see data in form of a table</div>
</div>
