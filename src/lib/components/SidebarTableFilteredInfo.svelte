...
/* let yearStatsSorted = $derived(yearStats ? yearStats.sort((a, b) => a.count - b.count) : null); */
    ...
    let highchartsValleyBarConfigModified = $derived.by(() =>
        ...
        {
        if (valleysStatsSorted) {
            let m=valleysStats.map((item) => {
            return {
                category: item.valley,
                y: item.count
            }
            })
            console.log('valleysStats data', m,m.map((item) => item.category));
            highchartsBarConfig.xAxis.categories=m.map((item) => item.category);
            return {
                ...highchartsBarConfig,
                series: [{
                    name: 'Valleys stats',
                ...
                categories: m.map((item) => item.category),

            };
        }
        })
    let highchartsYearlyBarConfigModified = $derived.by(() =>
        {
            if (yearStatsSorted) {
            let m=yearStats.sort((a, b) => a.year - b.year).map((item) => {
            return {
                category: item.year,
                y: item.count
            }
            })
            console.log('m data', m,m.map((item) => item.category));
            highchartsBarConfig.xAxis.categories=m.map((item) => item.category);
            return {
                ...highchartsBarConfig,
                series: [{
    ...
    $effect(() => {
        console.log('valleysStats:', valleysStats);
        console.log('valleysStatsSorted:', valleysStatsSorted);
    ...
    {:else if stats}
        ...
        <div>
                ...
                <p>Year stats available: {Object.keys(yearStats).length} years</p>
                    ...
                    <Tabs.Root value="list" class="gap-0">
                        <Tabs.List>
                        ...
                        <Tabs.Content value="list">
                          ...
                          >
                            ...
                            <Tabs.Content value="chart">
                                <div use:highcharts={highchartsYearlyBarConfigModified} class="w-full h-80"></div>
                            </Tabs.Content>
                        </Tabs.Root>
                ...
                <Tabs.Root value="list">
                    ...
                    <Tabs.Content value="list">
                      ...
                      >
                        ...
                        <Tabs.Content value="chart">
                            <div use:highcharts={highchartsValleyBarConfigModified} class="w-full h-80"></div>
                        </Tabs.Content>
                    </Tabs.Root>
            {/if}
    {:else}
